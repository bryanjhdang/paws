...
export class TimeEntryService {
  ...
  startEntry(
    ...
    if (user.currentTimeEntry.earnedCoins != -1) {
      throw Error(
        "Unable to start an entry when user already has entry running"
      );
    }
    ...
    return new Promise<TimeEntry>((resolve, reject) => {
      if (user.currentTimeEntry.earnedCoins != -1) {
        let newTimeEntry = new TimeEntry(
          ...
          user.currentTimeEntry.name,
          0
        );
        newTimeEntry.earnedCoins =
          (newTimeEntry.endTime - newTimeEntry.startTime) % 1000;
        ...
        firestoreHelper.updateUser(user);
        firestoreHelper
          .createTimeEntry(user.id, newTimeEntry)
          .then((timeEntry) => {
            resolve(timeEntry);
          });
      } else {
  ...
  generateStatistics() {
    firestoreHelper.generateStatistics();
  }
}

const timeEntryService = new TimeEntryService(firestoreHelper, oAuthHelper);
